...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	//send mail notifiacations - first to guest
	...
	msg := models.MailData{
		...
		From:     "rohanjadhav32963296@gmail.com",
		Subject:  "Reservation Confirmation",
		Content:  htmlMessage,
		Template: "/basic.html",
	}
	m.App.MailChan <- msg
	//send mail notifiacations - to property owner
	htmlMessage = fmt.Sprintf(`
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	//assume that there is no month and year is specified
	now := time.Now()
	...
	currentYear, currentMoth, _ := now.Date()
	currentLocation := now.Location()
	firstOfMonth := time.Date(currentYear, currentMoth, 1, 0, 0, 0, 0, currentLocation)
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	_ = m.DB.UpdateProcessedForReservarion(id, 1)
	year := r.URL.Query().Get("y")
	month := r.URL.Query().Get("m")
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, room := range rooms {
		if room.ID == 1 || room.ID == 2 {
			...
			if !ok {
				log.Println("No block map found for room ID:", room.ID)
				continue
			} else {
				log.Println("Block map for room ID:", room.ID, "is", curMap)
			}

			// Handle removing blocks
			for date, restrictionID := range curMap {
				if restrictionID > 0 {
					if !form.Has(fmt.Sprintf("remove_block_%d_%s", room.ID, date), r) {
						err := m.DB.DeleteBlockByID(restrictionID)
						if err != nil {
							log.Println(err)
						}
	...
	for name := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			...
			blockDate, _ := time.Parse("2006-01-2", exploded[3])
