...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	//writing the form values recieved into the database
	newReservationID, err := m.DB.InsertReservation(reservation)
	//newReservationID will be used along other info to build a room restriction model
	//and send that to a function insertroomrestriction
	if err != nil {
		helpers.ServerError(w, err)
		return
	}
	...
	//making a roomrestriction when reservation is done
...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	...
	ed := reservation.EndDate.Format("2026-01-02")
	stringMap := make(map[string]string)
	stringMap["start_date"] = sd
	stringMap["end_date"] = ed
